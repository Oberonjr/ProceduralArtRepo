...
public class Extrude : MeshCreator {
	...
	public override void RecalculateMesh() {
		...
		for (int i = 0; i < points.Count; i++) {
			// TODO: Add uvs
			builder.AddVertex(new Vector3(points[i].x, points[i].y, 0), new Vector2(1/(float)points.Count, 1/(float)points.Count));
		}
		...
		for (int i = 0; i < points.Count; i++) {
			// TODO: Add uvs
			builder.AddVertex(new Vector3(points[i].x, points[i].y, height), new Vector2(i/(float)points.Count, i/(float)points.Count));
		}
		for (int t = 0; t < triangles.Count; t += 3) {
		...
		for (int i = 0; i < points.Count; i++) {
			...
			// TODO: Add uvs
			// front vertices:
			int v1 = builder.AddVertex(new Vector3(points[i].x, points[i].y, 0), new Vector2(i/(float)points.Count, i/(float)points.Count));
			int v2 = builder.AddVertex(new Vector3(points[j].x, points[j].y, 0), new Vector2(j/(float)points.Count, j/(float)points.Count));
			// back vertices:
			int v3 = builder.AddVertex(new Vector3(points[i].x, points[i].y, height), new Vector2(i/(float)points.Count, i/(float)points.Count));
			int v4 = builder.AddVertex(new Vector3(points[j].x, points[j].y, height), new Vector2(j/(float)points.Count, j/(float)points.Count));
			// Add quad:
